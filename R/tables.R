...
NBA_tbl_colr <- function(DF, COL) {
  ...
  color_cell <- function(COL) {
    ...
    color <- col_mapping[match(COL, names(col_mapping))]
...
NBA_tbl_comb <- function(DF, GROUP, THR, PRO){
  ...
  for (i in 1:(nrow(final_table) - 1)) {
    thr_name <- final_table %>%
      ...
      as.data.frame()
    thr_name <- thr_name[,1]
    if (!is.na(thr_name) && thr_name %in% names(col_mapping)) {
      tbl_final <- tbl_final %>%
        kableExtra::row_spec(i, background = col_mapping[thr_name])
    }
  }
  ...
  for (j in 1:length(protection_order)) {
    ...
    tbl_final <- tbl_final %>%
      kableExtra::column_spec(j+1 ,
                  background = ifelse(protection_column %in% names(col_mapping),
                                      col_mapping[as.character(protection_column)],
                                      "white"),
                  include_thead = TRUE,
                  bold = FALSE)
  }
